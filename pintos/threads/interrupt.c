...
/* FUNCTION을 호출하는 게이트를 만듭니다.

   게이트에는 설명자 권한 수준 DPL이 있습니다.
   프로세서가 DPL에 있을 때 의도적으로 호출될 수 있습니다.
   또는 낮은 번호의 벨소리.  
   게이트를 호출하고 DPL==0은 이러한 호출을 방지합니다.  
   사용자 모드에서 발생하는 예외로 인해 여전히 게이트가 발생합니다.
   DPL==0이 호출됩니다.

   TYPE은 14(인터럽트 게이트의 경우) 또는 15(인터럽트 게이트의 경우)여야 합니다.
   트랩 게이트).  
   인터럽트를 비활성화하지만 트랩 게이트에 들어가는 것은 그렇지 않습니다.  
...
/* 외부 인터럽트는 외부 장치에 의해 생성되는 인터럽트입니다.
   ...
   선점.  
   intr_yield_on_return()을 호출하여 잠을 자더라도
...
intr_enable (void) {
	...
	/* 인터럽트 플래그를 설정하여 인터럽트를 활성화합니다.
	   ...
	   [IA32-v2b] "STI" 및 [IA32-v3a] 5.8.1 "마스킹 마스크 가능"을 참조하세요.
	   하드웨어 인터럽트". */
	asm volatile ("sti");
...
intr_disable (void) {
	...
	/* 인터럽트 플래그를 지워 인터럽트를 비활성화합니다.
	   [IA32-v2b] "CLI" 및 [IA32-v3a] 5.8.1 "마스킹 마스크 가능"을 참조하세요.
	   하드웨어 인터럽트". */
...
/* 설명자와 함께 HANDLER를 호출하기 위해 인터럽트 VEC_NO를 등록합니다.
   특권 수준 DPL.  
   목적.  
   인터럽트 상태가 LEVEL로 설정되었습니다. */
static void
...
/* HANDLER를 호출하기 위해 외부 인터럽트 VEC_NO를 등록합니다.
   디버깅 목적으로 NAME이라는 이름이 지정되었습니다.  
...
/* HANDLER를 호출하기 위해 내부 인터럽트 VEC_NO를 등록합니다.
   디버깅 목적으로 NAME이라는 이름이 지정되었습니다.  
   인터럽트 상태 LEVEL로 호출됩니다.

   핸들러는 설명자 권한 수준 DPL을 갖습니다.
   프로세서가 실행 중일 때 의도적으로 호출될 수 있습니다.
   DPL 또는 낮은 번호의 링.  
   인터럽트를 호출하는 사용자 모드와 DPL==0은 이러한 것을 방지합니다.
   기도.  
   여전히 DPL==0인 인터럽트가 호출됩니다.  
...
/* 모든 PC에는 2개의 8259A PIC(프로그램 가능 인터럽트 컨트롤러)가 있습니다.
   작은 조각.  
   다른 하나는 마스터의 IRQ 2 라인에 계단식으로 연결된 "슬레이브"입니다.
   포트 0xa0 및 0xa1에서 액세스할 수 있습니다.  
   A0 라인을 0으로 설정하고 0x21에 액세스하여 A1 라인을 다음으로 설정합니다.
   1. 상황은 슬레이브 PIC와 유사합니다.

   기본적으로 PIC가 전달하는 인터럽트 0~15는 다음으로 이동합니다.
   인터럽트 벡터 0...15.  
   CPU 트랩 및 예외에도 사용됩니다.  
   인터럽트 0~15가 인터럽트 벡터로 전달되도록
   대신 32...47(0x20...0x2f)입니다. */

/* PIC를 초기화합니다.   */
static void
pic_init (void) {
	/* 두 PIC의 모든 인터럽트를 마스크합니다. */
	outb (0x21, 0xff);
	outb (0xa1, 0xff);
